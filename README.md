...
互動式飛機百科網站：機型列表 → 3D 檢視器，點選機體部位顯示解說與規格。
純靜態網站，無需後端即可部署。
...
```
aircraft-archive/
├── index.html          機型列表頁（讀取 data/fleet.json）
...
靜態參考頁，整理機庫檔案 40 個條目所屬的機型家族與完整次型號系譜
...
獨立於機型頁面之外的第二個介面，收錄全球機場與跑道資料，可從任一
頁面的工具列切換。

資料來源：[OurAirports](https://ourairports.com/data/)（Public Domain，
`davidmegginson/ourairports-data` 每日更新的鏡像）。篩選規則：
`large_airport` / `medium_airport`，或任何有 IATA 代號的機場——涵蓋
全球具定期航班等級的機場，排除數萬個無代號的小型農用/私人跑道，
目前共 9,772 座機場、11,487 條跑道。

```
data/
├── airports.json          機場輕量索引（搜尋/列表用，~1.8 MB）
├── countries.json         國家代碼 → 名稱
└── runways/<ISO國碼>.json  依國家分桶的跑道明細（點開機場時才載入）
```
...
跑道明細依機場所屬國家分桶（而非每座機場一個檔案），單一國家最大的
檔案（美國，2,080 座機場）約 670 KB，其餘多在數十 KB 內，點開機場時
才依需求載入，不影響列表頁初始載入速度。
