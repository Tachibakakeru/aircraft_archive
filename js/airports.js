...
async function loadHubIndex(){
  ...
  HUB_INDEX = {};
  try {
    const res = await fetch("data/airline_geo.json?v=93");
...
async function loadAirlineNames(){
  ...
  AIRLINE_NAMES = {};
  try {
    const res = await fetch("data/airlines.json?v=93");
...
(async () => {
  try {
    const [aRes, cRes] = await Promise.all([
      fetch("data/airports.json?v=93"),
      fetch("data/countries.json?v=93"),
    ]);
  ...
  try {
    const res = await fetch("data/city_names.json?v=93");
...
async function loadDetails(country){
  ...
  try {
    const r = await fetch(`data/details/${encodeURIComponent(key)}.json?v=93`);
...
async function fetchPublished(id){
  ...
  try {
    const r = await fetch(`data/airport-notes/${encodeURIComponent(id)}.json?v=93`);
