...
async function loadHubIndex(){
  ...
  HUB_INDEX = {};
  try {
    const res = await fetch("data/airline_geo.json?v=84");
...
(async () => {
  try {
    const [aRes, cRes] = await Promise.all([
      fetch("data/airports.json?v=84"),
      fetch("data/countries.json?v=84"),
    ]);
  ...
  try {
    const res = await fetch("data/city_names.json?v=84");
...
async function loadDetails(country){
  ...
  try {
    const r = await fetch(`data/details/${encodeURIComponent(key)}.json?v=84`);
...
async function fetchPublished(id){
  ...
  try {
    const r = await fetch(`data/airport-notes/${encodeURIComponent(id)}.json?v=84`);
