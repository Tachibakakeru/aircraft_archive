...
NAME_RULES = [
    ...
    # 可動操縱面（逐片拆出供動畫）— flap fairing 整流罩維持靜態歸主翼
...
def convert(src, dst):
    ...
    # 將一組 primitive 依材質合併、去重量化 → entries（可重用於部位與單片操縱面）
    ...
    for pid, plist in part_prims.items():
        if pid in ANIMATABLE:
            ...
            for node, nplist in bynode.items():
                ...
                Pb = np.vstack([bake(p["pos"]) for p in nplist])
                axis, pivot, side = compute_hinge(Pb, pid)
